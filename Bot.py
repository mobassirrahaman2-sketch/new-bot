...
ADMIN_ID = 8634645150  # আপনার অ্যাডমিন চ্যাট আইডি ফিক্সড করা আছে

bot = telebot.TeleBot(TOKEN)

# --- ডাটাবেজ সেটআপ (JSON ফাইল নিরাপদ রাখার ব্যবস্থা) ---
DB_FILE = 'db.json'
...
def how_to_work(message):
    ...
    bot.send_message(message.chat.id, "পেছনে যেতে নিচের '🔙 ব্যাক' বাটনে চাপুন:", reply_markup=back_markup)
...
def handle_all_inputs(message):
    ...
    # 💰 সিক্রেট অ্যাডমিন কমান্ড: ব্যালেন্স অ্যাড করার ফিচার
    if text.startswith('/addmoney') and chat_id == ADMIN_ID:
        try:
            ...
            if target_id in db["users"]:
                ...
                bot.send_message(ADMIN_ID, "❌ এই আইডিটি ডাটাবেজে পাওয়া যায়নি! ইউজারকে অন্তত একবার বটে /start চাপতে বলুন।")
        except Exception:
            bot.send_message(ADMIN_ID, "⚠️ ফরম্যাট ভুল হয়েছে! এভাবে লিখুন: `/addmoney ইউজার_আইডি পরিমাণ`\nযেমন: `/addmoney 7958921197 300`", parse_mode='Markdown')
        return
    ...
    if current_step == 'INPUT_UID':
        ...
        # 👈 ফিক্সড: অ্যাডমিনের কাছে যাওয়ার নোটিফিকেশনে "পেমেন্ট সম্পন্ন করুন" বাটন যোগ করা হলো
...
def handle_callback(call):
    ...
    # 👈 নতুন ফিচার: "পেমেন্ট সম্পন্ন করুন" বাটনে চাপ দিলে ইউজারের কাছে টাকার পরিমাণসহ মেসেজ যাবে
    elif call.data.startswith('pay_') and chat_id == ADMIN_ID:
        try:
            _, target_user_id, paid_amount = call.data.split('_')
            
            # ইউজারকে সফল পেমেন্টের মেসেজ পাঠানো
            success_msg = f"🎉 *আপনার টাকা সফলভাবে আপনাকে দেওয়া হয়েছে।*\n💰 পরিমাণ: *{paid_amount} ৳*"
            bot.send_message(target_user_id, success_msg, parse_mode='Markdown')
            
            # অ্যাডমিনকে কনফার্ম করা এবং বাটনটি রিমুভ করে টেক্সট আপডেট করা
            bot.edit_message_text(
                chat_id=ADMIN_ID,
                message_id=call.message.message_id,
                text=call.message.text + f"\n\n✅ **[পেমেন্ট সম্পূর্ণ করা হয়েছে এবং ইউজারকে নোটিফিকেশন পাঠানো হয়েছে]**",
                reply_markup=None  # বাটন ডিলিট করে দেওয়া হলো
            )
        except Exception:
            bot.answer_callback_query(call.id, "❌ পেমেন্ট কনফার্মেশনে কোনো সমস্যা হয়েছে!")
